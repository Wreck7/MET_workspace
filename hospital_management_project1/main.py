patient_database = {'P002': {'name': 'Varun', 'age': '66', 'gender': 'Male', 'blood_group': 'A-', 'insurance_name': 'India', 'insurance_id': 'ii9000'}}


# generating a unique patient id 
def generate_id():
    pid = len(patient_database) + 1
    return f'P{pid:03}'
...
def register_patient(patient_id, personal_info, insurance_info): #  (medical_history) missed this cauz how can i say this
    patient_id_keys = list(patient_database.keys())
    for i in range(len(patient_database)):
        if patient_id_keys[i] == patient_id and patient_database.get(patient_id_keys[i]).get('name') == personal_info.get('name'):
            return f'{patient_id} and {personal_info.get('name')} already exists in records!'
        break
